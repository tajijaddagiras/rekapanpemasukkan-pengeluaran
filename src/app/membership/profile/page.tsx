      ...
      <div className="bg-white rounded-[20px] md:rounded-[40px] border border-slate-50 shadow-sm p-5 md:p-10 lg:p-12">
        <div className="flex flex-col lg:flex-row items-center gap-6 lg:gap-10">
          <div className="relative group shrink-0">
            ...
            <button className="absolute -bottom-1 -right-1 md:bottom-2 md:right-2 w-10 h-10 md:w-11 md:h-11 rounded-xl md:rounded-2xl bg-white shadow-lg border border-slate-100 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all hover:scale-110 active:scale-95">
              <Camera size={18} />
            </button>
          </div>
          ...
          <div className="flex-1 text-center lg:text-left">
            <div className="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-5 mb-3">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-black text-slate-900 tracking-tight">John Doe</h1>
        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          <div className="bg-white p-5 md:p-8 lg:p-12 rounded-[20px] md:rounded-[48px] border border-slate-50 shadow-sm space-y-6 md:space-y-10">
              ...
              <div className="space-y-4">
                ...
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50/50 border-none focus:ring-2 focus:ring-indigo-100 rounded-xl md:rounded-2xl py-4 md:py-5 pl-14 pr-6 text-sm font-bold text-slate-700 transition-all placeholder:text-slate-300"
                  />
                </div>
              </div>
              ...
              <div className="md:col-span-2 space-y-4">
                ...
                <div className="relative group">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-slate-50/50 border-none focus:ring-2 focus:ring-indigo-100 rounded-xl md:rounded-2xl py-4 md:py-5 pl-14 pr-6 text-sm font-bold text-slate-700 transition-all placeholder:text-slate-300 resize-none"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="pt-8 md:pt-10 border-t border-slate-50 flex flex-col md:flex-row items-center gap-4 md:gap-6">
              <button className="w-full md:w-fit px-10 md:px-12 py-4 md:py-5 bg-indigo-600 text-white rounded-xl md:rounded-2xl text-[13px] md:text-xs font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all hover:translate-y-[-2px] active:translate-y-0">
                Save All Updates
              </button>
              <button className="w-full md:w-fit px-8 py-4 md:py-5 bg-transparent text-slate-400 rounded-xl md:rounded-2xl text-[13px] md:text-xs font-black border border-slate-50 hover:bg-slate-50 transition-all">
                Cancel
              </button>
            </div>
          </div>
          ...
          <div className="bg-[#f0f5fa] p-5 md:p-8 rounded-[20px] md:rounded-[48px] border border-white shadow-sm space-y-6 md:space-y-8">
            ...
            <div className="space-y-4">
              {bankAccounts.map((bank, i) => (
                <div key={i} className="flex items-center justify-between p-4 md:p-5 bg-white/60 backdrop-blur-sm rounded-[24px] md:rounded-[28px] border border-white shadow-sm hover:scale-[1.02] transition-transform cursor-pointer">
                  <div className="flex items-center gap-3 md:gap-4">
                    <div className={`w-10 h-10 md:w-11 md:h-11 rounded-xl md:rounded-2xl flex-shrink-0 ${bank.color} text-white flex items-center justify-center text-[10px] font-black shadow-lg shadow-blue-100`}>
                      {bank.icon}
                    </div>
                    <div>
                      <h4 className="text-[12px] md:text-[13px] font-black text-slate-900 leading-tight">{bank.name}</h4>
                      <p className="text-[9px] md:text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tight">{bank.type}</p>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-[12px] md:text-[13px] font-black text-slate-900">Rp {bank.balance}</p>
                    <ArrowRight size={12} className="text-slate-300 float-right mt-1" />
                  </div>
                </div>
              ))}
            </div>
            <button className="w-full py-3.5 md:py-4 rounded-xl md:rounded-2xl border-2 border-dashed border-indigo-200 text-indigo-500 text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-white/50 transition-all">
              + Connect New Bank
            </button>
          </div>
              ...
              <div className="pt-6 border-t border-white/10">
                <div className="flex items-center justify-between">
                  ...
                  <div className="w-10 h-5 md:w-12 md:h-6 bg-indigo-600 rounded-full relative p-1 cursor-pointer">
                    <div className="w-3 h-3 md:w-4 md:h-4 bg-white rounded-full absolute right-1"></div>
                  </div>
                </div>
              </div>
            </div>
