...
export default function MarketDataPage() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 max-w-[1240px] mb-20">
      
      {/* Header */}
      <div>
        <h1 className="text-4xl font-black text-slate-900 tracking-tight">Dashboard Intelligence</h1>
        <p className="text-sm font-medium text-slate-500 mt-2">Curated financial data and market whitelists for decisive actions.</p>
      </div>
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="bg-white p-8 rounded-[32px] border border-slate-50 shadow-sm space-y-8">
            ...
            <div className="space-y-6">
              ...
              <button className="w-full bg-indigo-600 text-white py-4 rounded-xl text-xs font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95">
                Simpan Rate
              </button>
            </div>
          </div>
          ...
          <div className="bg-slate-50/50 p-6 rounded-[32px] border border-slate-100 space-y-6">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="bg-white p-5 rounded-2xl shadow-sm border border-white space-y-4">
                  <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">{item.title}</p>
                  <div className="bg-slate-50 px-4 py-2.5 rounded-lg text-[10px] font-bold text-slate-600 leading-relaxed font-mono">
                    {item.tags}
                  </div>
                  <button className="flex items-center gap-2 text-[9px] font-black text-indigo-600 uppercase tracking-widest hover:underline">
        ...
        <div className="lg:col-span-9 space-y-8">
          ...
          <div className="bg-white rounded-[32px] border border-slate-50 shadow-sm overflow-hidden">
            <div className="p-8 pb-4 flex items-center justify-between border-b border-slate-50">
              <h2 className="text-base font-black text-slate-900 tracking-tight">Exchange Rate Table</h2>
              <span className="px-3 py-1 bg-cyan-50 text-cyan-600 text-[9px] font-black rounded-full uppercase tracking-widest flex items-center gap-2">
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">
                    <th className="py-6 font-black">Dari</th>
                    <th className="py-6 font-black text-center">Simbol</th>
                    <th className="py-6 font-black text-center">Ke</th>
                    <th className="py-6 font-black text-right">Rate</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {currencies.map((curr, i) => (
                    <tr key={i} className="group hover:bg-slate-50/50 transition-colors">
                      <td className="py-6 text-[13px] font-bold text-slate-900">{curr.from}</td>
                      <td className="py-6 text-center text-indigo-500"><ArrowRight size={16} className="mx-auto" /></td>
                      <td className="py-6 text-center text-[13px] font-bold text-slate-900 pr-12">{curr.to}</td>
                      <td className="py-6 text-right text-[14px] font-black text-slate-900 tracking-tight">{curr.rate}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-8 rounded-[32px] border border-slate-50 shadow-sm space-y-8">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-black text-slate-900 tracking-tight">Foreign Exchange</h3>
                ...
                {forex.map((item, i) => (
                  <div key={i} className="flex items-center justify-between group cursor-pointer">
                    <div>
                      ...
                      <p className="text-[10px] font-medium text-slate-400 tracking-tight">{item.label}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[13px] font-black text-slate-900">{item.price}</p>
                      <p className={`text-[10px] font-bold mt-0.5 ${item.isDown ? 'text-rose-500' : 'text-emerald-500'}`}>{item.change}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white p-8 rounded-[32px] border border-slate-50 shadow-sm space-y-8">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-black text-slate-900 tracking-tight">Cryptocurrency</h3>
                <span className="text-[9px] font-bold text-slate-400">Updated: Just now</span>
              </div>
              <div className="space-y-6">
                {crypto.map((item, i) => (
                  <div key={i} className="flex items-center justify-between group cursor-pointer">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 font-black text-[10px] border border-slate-100 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">
                        {item.icon}
                      </div>
                      <div>
                        <h4 className="text-[13px] font-black text-slate-900 tracking-tight">{item.name}</h4>
                        <p className="text-[10px] font-medium text-slate-400 tracking-tight uppercase">{item.symbol}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-[13px] font-black text-slate-900">{item.price}</p>
          ...
          <div className="bg-white p-8 lg:p-10 rounded-[40px] border border-slate-50 shadow-sm">
