            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="relative">
                  <select className="w-full appearance-none bg-slate-50 border-none focus:ring-2 focus:ring-slate-100 rounded-xl py-3 md:py-4 px-6 text-sm font-medium text-slate-600 transition-all cursor-pointer">
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <input 
                    type="text" 
                    placeholder="0.00"
                    className="w-full bg-slate-50 border-none focus:ring-2 focus:ring-slate-100 rounded-xl py-3 md:py-4 pl-14 pr-6 text-sm font-medium text-slate-600 placeholder:text-slate-300 transition-all"
                  />
                </div>
              </div>
 
              <button className="w-full bg-[#004d40] flex items-center justify-center gap-3 py-3 md:py-4 rounded-2xl text-xs font-black text-white hover:bg-[#002f26] transition-all shadow-lg shadow-green-50">
                <Save size={16} />
                Simpan Budget
              </button>
            </div>
          ...
          <div className="bg-white h-full rounded-[20px] md:rounded-[40px] border border-slate-50 shadow-sm p-5 md:p-10 flex flex-col">
            ...
            <div className="flex-1 space-y-2">
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between p-3 md:p-4 rounded-2xl bg-slate-50/50 border border-slate-50">
                  <div className="flex items-center gap-3 md:gap-4">
                    <div className={`w-10 h-10 rounded-xl shrink-0 ${item.bg} ${item.iconColor} flex items-center justify-center border border-white shadow-sm`}>
                      <item.icon size={18} />
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-[12px] md:text-[13px] font-black text-slate-900 tracking-tight truncate">{item.title}</h4>
                      <p className="text-[9px] md:text-[10px] font-medium text-slate-400 mt-0.5 truncate">{item.desc}</p>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-[12px] md:text-[13px] font-black text-slate-900 tracking-tight">Rp {item.amount}</p>
                    <p className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{item.type}</p>
                  </div>
                </div>
              ))}
            </div>
